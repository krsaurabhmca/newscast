...
$unread_count = 0;
try {
    $unread_count = $pdo->query("SELECT COUNT(*) FROM feedback WHERE status='pending'")->fetchColumn();
} catch (Exception $e) {
}
...
<style>
   ...
   ════════════════════════════════ */
    ...
    .sb-nav ul li a:hover .sbi {
        background: rgba(255, 255, 255, .08);
    }

    .sb-nav ul li a.active .sbi {
        background: color-mix(in srgb, var(--primary) 25%, transparent);
    }

    .sb-nav ul li a .sbi svg {
        width: 15px;
        height: 15px;
    }
    ...
    .sidebar-collapsed .sb-nav ul li a {
        justify-content: center;
        padding: 9px 0;
    }

    .sidebar-collapsed .sb-nav ul li a::before {
        display: none;
    }

    /* Tooltip on collapsed */
    .sidebar-collapsed .sb-nav ul li {
        position: relative;
    }

    .sidebar-collapsed .sb-nav ul li a .sbi {
        margin: 0;
    }
</style>
            ...
            <ul>
                ...
                <li>
                    <a href="system_update.php" class="<?php echo sb_active('system_update.php'); ?>">
                        <span class="sbi" style="background:rgba(59,130,246,.15);"><i data-feather="download-cloud"
                                style="color:#3b82f6;"></i></span>
